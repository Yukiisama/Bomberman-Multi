...
class Bomb:
    def __init__(self, m, pos, time_to_explode):
        self.map = m
        self.pos = pos
        self.max_range = MAX_RANGE
        self.countdown = (time_to_explode/1000)-1
        self.time_to_explode = time_to_explode # in ms
        # compute bomb range
...
class Character:
        ...
        if direction == DIRECTION_RIGHT:
            if self.pos[X] < (self.map.width - 1):
                if self.map.array[self.pos[Y]][self.pos[X] + 1] not in WALLS:
                    bomb_is_there = False
                    for bomb in bombs:
                        if self.pos[X]+1 == bomb.pos[X] and self.pos[Y] == bomb.pos[Y] and bomb.countdown != 0:
                            bomb_is_there = True
                    if not bomb_is_there: self.pos = (self.pos[X]+1, self.pos[Y])
            self.direction = DIRECTION_RIGHT
        # move left
        ...
        # move up
            ...
            if self.pos[Y] > 0:
                if self.map.array[self.pos[Y] - 1][self.pos[X]] not in WALLS:
                    bomb_is_there = False
                    for bomb in bombs:
                        if self.pos[X] == bomb.pos[X] and self.pos[Y]-1 == bomb.pos[Y] and bomb.countdown != 0:
                            bomb_is_there = True
                    if not bomb_is_there: self.pos = (self.pos[X], self.pos[Y]-1)
    ...
    def eat(self, fruit):
        ...
        #    self.health += 10
         #   ("{}\'s health: {}".format(self.nickname, self.health))
          #  return True
        if fruit.kind==CHERRY and fruit.pos[X] == self.pos[X] and fruit.pos[Y] == self.pos[Y]:
            self.health += 10
            self.immunity = 500
            return True
        if fruit.kind==BANANA and fruit.pos[X] == self.pos[X] and fruit.pos[Y] == self.pos[Y]:
            freecase = False
            while not freecase:
                x, y = random.randint(0, self.map.width-1),random.randint(0,self.map.height-1)
                if self.map.array[y][x] not in WALLS:
                    self.pos=x,y
                    self.disarmed=5
                    freecase = True
...
class Model:
    ...
    def drop_bomb(self, nickname):
        ...
        if character.disarmed == 0:
            self.bombs.append(Bomb(self.map, character.pos, 5999))
    ...
    def move_character(self, nickname, direction):
        ...
        if not character:
            print("Error: nickname \"{}\" not found!".format(nickname))
            sys.exit(1)
        character.move(direction, Bombs)
